...
<html lang="cs">
    ...
    <body class="bg-stone-50 text-stone-900 antialiased">
        <header class="border-b border-teal-100 bg-white/90 backdrop-blur">
            <div class="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
                <div>
                    <p class="text-sm font-semibold uppercase tracking-[0.3em] text-teal-700">Arytmie Praha</p>
                    <p class="text-sm text-stone-600">Ženský komorní sbor</p>
                </div>

                <nav class="hidden gap-6 text-sm font-medium text-stone-700 md:flex">
                    <a href="#o-sboru" class="transition hover:text-teal-700">O sboru</a>
                    <a href="#aktuality" class="transition hover:text-teal-700">Aktuality</a>
                    <a href="#koncerty" class="transition hover:text-teal-700">Koncerty</a>
                    <a href="{{ route('gallery.index') }}" class="transition hover:text-teal-700">Galerie</a>
                    <a href="#kontakt" class="transition hover:text-teal-700">Kontakt</a>
                </nav>
            </div>
        </header>
        ...
        <main>
            <section class="bg-gradient-to-b from-stone-200/80 to-stone-50">
                <div class="mx-auto grid max-w-6xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1.2fr_0.8fr] lg:px-8 lg:py-24">
                    <div class="space-y-6">
                        <p class="text-sm font-semibold uppercase tracking-[0.3em] text-teal-700">Nový web ve výstavbě</p>
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row">
                            <a
                                href="#koncerty"
                                class="rounded-full bg-teal-700 px-6 py-3 text-center text-sm font-semibold text-white transition hover:bg-teal-800"
                            >
                                Zobrazit koncerty
                            </a>
                            <a
                                href="#kontakt"
                                class="rounded-full border border-teal-200 bg-white px-6 py-3 text-center text-sm font-semibold text-teal-800 transition hover:border-teal-300 hover:bg-teal-50"
                            >
                                Kontaktovat sbor
                            </a>
                        </div>
                    ...
                    <div class="rounded-3xl border border-teal-200 bg-white p-6 shadow-sm">
                        <p class="text-sm font-semibold uppercase tracking-[0.2em] text-teal-700">Co už je připraveno</p>
                        <ul class="mt-4 space-y-3 text-sm text-stone-700">
                            <li class="rounded-2xl bg-teal-50 px-4 py-3">Laravel 13 + Livewire 4 běží na `newarytmie.test`.</li>
                            <li class="rounded-2xl bg-teal-50 px-4 py-3">Databáze je přepnutá na MySQL v Laragonu.</li>
                            <li class="rounded-2xl bg-teal-50 px-4 py-3">Hotový je základ datového modelu pro obsah, koncerty i galerii.</li>
                        </ul>
                    </div>
                </div>
            </section>
            ...
            <section id="o-sboru" class="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
                <div class="grid gap-8 lg:grid-cols-2">
                    <div>
                        <p class="text-sm font-semibold uppercase tracking-[0.3em] text-teal-700">O sboru</p>
            ...
            <section id="aktuality" class="bg-white">
                <div class="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
                    <div class="flex items-end justify-between gap-6">
                        <div>
                            <p class="text-sm font-semibold uppercase tracking-[0.3em] text-teal-700">Aktuality</p>
                        ...
                        @forelse ($newsItems as $news)
                            <article class="rounded-3xl border border-stone-200 bg-stone-50 p-6">
                                ...
                                <h3 class="mt-2 text-xl font-semibold">
                                    <a href="{{ route('news.show', $news->slug) }}" class="hover:text-teal-700">
                                        {{ $news->title }}
                                    </a>
                                </h3>
                                ...
                                <a href="{{ route('news.show', $news->slug) }}" class="mt-4 inline-block text-sm font-semibold text-teal-700 hover:text-teal-800">
                                    Cist aktualitu &rarr;
                                </a>
                            </article>
                        @empty
                            <article class="rounded-3xl border border-stone-200 bg-stone-50 p-6 md:col-span-3">
                                <p class="text-sm text-teal-700">Zatím bez příspěvků</p>
            ...
            <section id="koncerty" class="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
                <div class="rounded-[2rem] bg-stone-900 px-6 py-8 text-white lg:px-10">
                    <p class="text-sm font-semibold uppercase tracking-[0.3em] text-teal-300">Koncerty</p>
                    ...
                    <div class="mt-8 grid gap-4 md:grid-cols-3">
                        @forelse ($concertItems as $concert)
                            <div class="rounded-3xl bg-white/10 p-5">
                                <p class="text-sm text-teal-200">{{ $concert->starts_at?->format('d.m.Y H:i') }}</p>
                                <p class="mt-2 text-lg font-semibold">
                                    <a href="{{ route('concerts.show', $concert->slug) }}" class="hover:text-teal-200">
                                        {{ $concert->title }}
                                    </a>
                                </p>
                                <p class="mt-2 text-sm text-teal-100">{{ $concert->venue_name }}, {{ $concert->city }}</p>
                                @if ($concert->program)
                                    <p class="mt-2 text-xs text-teal-100">
                                        {{ \Illuminate\Support\Str::limit($concert->program, 90) }}
                                    </p>
                                @endif
                                <a href="{{ route('concerts.show', $concert->slug) }}" class="mt-3 inline-block text-sm font-semibold text-teal-200 hover:text-white">
                                    Detail koncertu &rarr;
                                </a>
                            </div>
                        @empty
                            <div class="rounded-3xl bg-white/10 p-5 md:col-span-3">
                                <p class="text-sm text-teal-200">Zatím bez termínů</p>
            ...
            <section id="galerie" class="bg-white">
                <div class="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
                    <p class="text-sm font-semibold uppercase tracking-[0.3em] text-teal-700">Galerie</p>
                    <h2 class="mt-3 text-3xl font-semibold tracking-tight">Fotografie budou rozdělené do alb</h2>
                    <div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        <div class="aspect-square rounded-3xl bg-teal-100"></div>
                        <div class="aspect-square rounded-3xl bg-teal-200"></div>
                    ...
                    <a href="{{ route('gallery.index') }}" class="mt-6 inline-block text-sm font-semibold text-teal-700 hover:text-teal-800">
            ...
            <section id="kontakt" class="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
                <div class="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
                    <div class="space-y-4">
                        <p class="text-sm font-semibold uppercase tracking-[0.3em] text-teal-700">Kontakt</p>
                    ...
                    <div class="rounded-[2rem] border border-stone-200 bg-white p-6 shadow-sm">
                        <form class="grid gap-4">
                            <input type="text" placeholder="Jméno" class="rounded-2xl border border-stone-300 px-4 py-3 outline-none transition focus:border-teal-400">
                            <input type="email" placeholder="E-mail" class="rounded-2xl border border-stone-300 px-4 py-3 outline-none transition focus:border-teal-400">
                            <input type="text" placeholder="Předmět" class="rounded-2xl border border-stone-300 px-4 py-3 outline-none transition focus:border-teal-400">
                            <textarea rows="5" placeholder="Vaše zpráva" class="rounded-2xl border border-stone-300 px-4 py-3 outline-none transition focus:border-teal-400"></textarea>
